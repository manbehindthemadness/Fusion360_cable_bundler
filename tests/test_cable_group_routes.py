"""
Tests for topology-derived cable-group preview planning.
"""
...
from cable_bundler.application import CableGroupControlStep, plan_cable_group_routes
...
def test_plans_one_deterministic_leg_for_two_grouped_ends(
    ...
    forward = replace(
        valid_harness,
        cable_groups=(CableGroupDefinition(group_id, group.connection_ids),),
    )
    reverse = replace(
        forward,
        cable_groups=(CableGroupDefinition(group_id, tuple(reversed(group.connection_ids))),),
    )

    forward_leg = plan_cable_group_routes(forward)[0]
    reverse_leg = plan_cable_group_routes(reverse)[0]
...
def test_plans_terminal_lead_from_internal_pathway_boundary(
    ...
    legs = plan_cable_group_routes(attached)
    ...
    assert lead.control_steps == (CableGroupControlStep(expected_control_id, expected_reversed),)
    assert sorted(pathway_id for leg in legs for pathway_id in leg.pathway_ids) == sorted(
        pathway_ids
    )
...
def test_rejects_terminal_lead_without_a_pathway_control(
    ...
    with pytest.raises(ValueError, match="internal end.*without a routing control"):
        plan_cable_group_routes(replace(attached, pathways=tuple(pathways)))
...
def test_rejects_multi_end_branching_at_a_terminal_boundary(
    ...
    with pytest.raises(ValueError, match="junction-centered branching"):
        plan_cable_group_routes(linear)
...
def _with_internal_end(
    ...
    updated = replace(
        ...
        cable_groups=(replace(group, connection_ids=(*group.connection_ids, connection_id)),),
    )
...
def _y_harness(
    ...
    definition = replace(
        ...
        cable_groups=(CableGroupDefinition(UUID(int=970), connection_ids),),
    )
