"""
Expose the Cable Bundler lifecycle entry points to Fusion 360.
"""
...
def _unload_addin_package(addin: _AddinLifecycle) -> None:
    ...
    if not package_name:
        raise RuntimeError("Cable Bundler lifecycle module has no package name.")
...
def _report_import_failure(operation: str) -> None:
    ...
    if application and application.userInterface:
        application.userInterface.messageBox(
            f"Cable Bundler failed to {operation}:\n{traceback.format_exc()}",
            "Harness Builder",
        )
