/** Connectivity and staged assignment model for cable creation. */

/** Coordinate pathway-boundary selection and the planned cable-creation workspace. */

let cancelActiveCableCreationSelection = null;

/** Return the stable graph identity for one pathway boundary. */
function cableCreationBoundaryKey(pathwayId, endpoint) {
  return `pathway:${pathwayId}:${endpoint}`;
}

/** Return every pathway boundary physically reachable from one boundary. */
function cableCreationReachableBoundaries(harness, sourceKey) {
  const graph = new Map();
  const addNode = (key) => {
  ...
  harness.pathways.forEach((pathway) => connect(
    cableCreationBoundaryKey(pathway.pathwayId, "start"),
    cableCreationBoundaryKey(pathway.pathwayId, "end"),
  ));
  (harness.junctions || []).forEach((junction) => {
    ...
    (junction.pathwayRelationships || []).forEach((relationship) => connect(
      junctionKey,
      cableCreationBoundaryKey(relationship.pathwayId, relationship.endpoint),
    ));
  });
...
function cableCreationBoundaryLabel(boundary) {
...
function cableCreationBoundaryState(boundary) {
...
function resolveCableCreationBoundary(harness, state) {
  ...
  return {
    key: cableCreationBoundaryKey(pathway.pathwayId, state.endpoint),
    pathway,
...
function highlightCableCreationEnd(harness, group) {
  if (group.cableGroupId) {
    return highlightMember(harness, "cable_group", group.cableGroupId);
  }
...
/** Create the transient ordered state for a newly opened cable workspace. */
function initialCableCreationAssignments(groups) {
  const rightByGroup = new Map(
    groups.right.filter((group) => group.cableGroupId).map((group) => [
      group.cableGroupId, group,
    ]),
  );
  ...
  groups.left.forEach((left) => {
    const right = left.cableGroupId && rightByGroup.get(left.cableGroupId);
    if (!right) return;
...
function reconcileCableCreationAssignments(assignments, groups) {
  if (!assignments) return initialCableCreationAssignments(groups);
  ...
  if (pendingRows.length > 1) {
    returnOtherPendingCableCreationRows(assignments, newlyPendingRow || pendingRows[0]);
  }
  return assignments;
}

/** Insert an item into its outer pool without exceeding the current list bounds. */
function insertCableCreationPoolItem(assignments, side, item, index) {
...
function returnOtherPendingCableCreationRows(assignments, keepRow) {
  assignments.rows = assignments.rows.filter((row) => {
    ...
    const item = row[side];
    insertCableCreationPoolItem(assignments, side, item, item.returnIndex);
    return false;
  });
}

/** Move one outer-pool card into a center row. */
function assignCableCreationPoolItem(assignments, side, poolIndex, rowIndex) {
  ...
  if (pending && pending[side]) {
    insertCableCreationPoolItem(assignments, side, pending[side], pending[side].returnIndex);
...
function unassignCableCreationRowItem(assignments, side, rowIndex, poolIndex) {
  ...
  insertCableCreationPoolItem(assignments, side, item, poolIndex);
  row[side] = null;
  if (!row.left && !row.right) assignments.rows.splice(rowIndex, 1);
  else returnOtherPendingCableCreationRows(assignments, row);
}

/** Exchange two occupied center-row members without moving their partners. */
function swapCableCreationRowItems(assignments, side, sourceRowIndex, targetRowIndex) {
...
function deleteCableCreationEnd(assignments, connectionId) {
  ...
  if (pendingRows.length > 1) returnOtherPendingCableCreationRows(assignments, pendingRows[0]);
}

/** Return the complete left/right relationships currently staged in the center. */
function cableCreationCompletePairings(assignments) {
...
function cableCreationDetachedConnectionIds(assignments, pairings, deleted) {
  const finalPartners = {};
...
function cableCreationStagedConnectedIds(harness, assignments) {
  ...
  const detached = new Set(
    cableCreationDetachedConnectionIds(assignments, pairings, deleted),
  );
  const groups = (harness.cableGroups || []).map((group) => (
