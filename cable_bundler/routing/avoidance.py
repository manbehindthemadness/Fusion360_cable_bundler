...
class RouteCollision:
    """
    Describe the worst remaining overlap between two distinct cable groups.
    """
    ...
    for _pass_index in range(_MAX_REPAIR_PASSES):
        ...
        for collision in sorted(
            ...
            candidates = sorted(
                ...
                key=lambda index: str(routes[index].cable_id),
                reverse=True,
            )
            for route_index in candidates:
                ...
                selected_point = (
                    ...
                    if current[route_index].cable_id == collision.left_route_id
...
def _route_geometry(
    ...
    if not capsules:
        raise ValueError(f"Cable {route.cable_number} has no measurable collision geometry.")
    ...
    for left_capsule in left.capsules:
        for right_capsule in right.capsules:
            ...
            candidate = RouteCollision(
                left.route.cable_id,
                right.route.cable_id,
                left.route.cable_number,
                right.route.cable_number,
                shortfall,
                left_point,
                right_point,
            )
