"""
Plan non-duplicated preview legs for connected wire-end groups.
"""
...
from .edit_wire_groups import wire_end_locations


@dataclass(frozen=True)
class WireGroupControlStep:
    """
...
@dataclass(frozen=True)
class WireGroupRouteLeg:
    """
    Describe one maximal terminal-or-junction leg of a wire-group tree.
    ...
    control_steps: tuple[WireGroupControlStep, ...]
...
def plan_wire_group_routes(definition: HarnessDefinition) -> tuple[WireGroupRouteLeg, ...]:
    """
    Convert every wire group into unique legs of its minimal topology tree.
    ...
    planned: list[WireGroupRouteLeg] = []
    for group_index, group in enumerate(definition.wire_groups):
        ...
        for connection_id in group.connection_ids:
            ...
            if location is None:
                raise ValueError(
                    f"Wire group {group_index + 1} contains an end without a pathway boundary."
                )
        ...
        if len(terminal_nodes) < 2:
            raise ValueError(f"Wire group {group_index + 1} requires at least two ends.")
        ...
        for leg_index, path in enumerate(paths):
            ...
            planned.append(
                WireGroupRouteLeg(
                    route_id=uuid5(group.wire_group_id, canonical_path),
...
def _topology_graph(
    ...
    """
    Build the undirected pathway/junction topology used by the Wire Editor.
    """
...
def _minimal_tree_edges(
    ...
    for target in terminals[1:]:
        ...
        if target not in visited:
            raise ValueError(
                f"Wire group {group_index + 1} contains ends in disconnected pathway networks."
            )
        node = target
        while node != root:
...
def _path_controls(
...
) -> tuple[tuple[WireGroupControlStep, ...], tuple[UUID, ...]]:
    """
    Expand one topology leg into traversal-ordered controls and pathways.
    """
    steps: list[WireGroupControlStep] = []
    ...
    for index, node in enumerate(path):
        if node.kind == "junction":
            ...
            steps.append(WireGroupControlStep(junction_controls[node.identity], reverse))
        if index + 1 >= len(path):
            continue
        ...
        steps.extend(WireGroupControlStep(control_id, not forward) for control_id in control_ids)
