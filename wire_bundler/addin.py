...
ADD_END_COMMAND_ID = "kev0_wire_bundler_add_end"
...
_pending_standalone_end_harness_id: Optional[UUID] = None
...
class _AddStandaloneEndCreatedHandler(adsk.core.CommandCreatedEventHandler):
    ...
    def notify(self, args: adsk.core.CommandCreatedEventArgs) -> None:
        ...
        """
        global _pending_standalone_end_harness_id

        harness_id = _pending_standalone_end_harness_id
        _pending_standalone_end_harness_id = None
        if harness_id is None:
        ...
        _handlers.extend((preselect, changed, validate, execute))
    ...
    def notify(self, args: adsk.core.HTMLEventArgs) -> None:
        ...
        try:
            ...
            if html_args.action == "add_junction_relationship":
                ...
                html_args.returnData = json.dumps({"ok": True})
                return
            if html_args.action == "add_end":
                _open_add_end_command(application, html_args.data)
                html_args.returnData = json.dumps({"ok": True})
                return
...
def start(_context: object) -> None:
    ...
    try:
        ...
        add_end_definition = user_interface.commandDefinitions.addButtonDefinition(
            ADD_END_COMMAND_ID,
            ADD_END_COMMAND_NAME,
            "Create a disconnected end at an existing pathway boundary.",
            ADD_WIRES_RESOURCE_FOLDER,
        )
        if add_end_definition is None:
            raise RuntimeError("Fusion did not create the Add End command definition.")
        add_end_handler = _AddStandaloneEndCreatedHandler()
        if not add_end_definition.commandCreated.add(add_end_handler):
            raise RuntimeError("Fusion did not register the standalone-end creation handler.")
        _handlers.append(add_end_handler)
...
def stop(_context: object) -> None:
    ...
    global _pending_junction_harness_id, _pending_standalone_end_harness_id
    global _pending_segment_ids
    global _pending_refine_edit_ids
    global _pending_end_edit
...
def _open_add_end_command(application: adsk.core.Application, serialized_data: str) -> None:
    ...
    _pending_standalone_end_harness_id = harness_id
    try:
        if not command_definition.execute():
